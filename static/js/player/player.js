...
function PlayState(){
  ...
  this.init = function(){
    setInterval(function(){ App.Player.update(); }, 50);
  };
  ...
  this.searchItems = function(searchText){
    ...
    if(Ui.playlist){
      Ui.playlist.render(this.playlist);
    } else {
      Ui.playlist = new PlaylistView();
      App.contentRegion.show(Ui.playlist);
    }
  };
  ...
  this.songSortFunc = function(a, b){
    if(a[player.sort_col] < b[player.sort_col])
  ...
  this.trackEnded = function(){
    ...
    App.router.songview.redrawSong(this.current_song.attributes._id);
  ...
  this.genShufflePool = function(){
    ...
    if(player.playlist && player.playlist._id == "QUEUE"){
      ...
      if(App.router.songview){
        App.router.songview.render();
      }
    }
  };
  ...
  this.prevTrack = function(){
    ...
    if(this.current_track.currentTime > 5.00 || this.repeat_state == this.repeat_states.one){
      ...
      if(this.play_history.length > 0 && this.play_history_idx+1 < this.play_history.length){
        ...
        this.playSong(this.queue_pool[index]._id, true);
      }
    }
  };
